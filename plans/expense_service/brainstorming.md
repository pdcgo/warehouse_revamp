...
> Planning doc for **`expense_service`**. **Nothing is implemented.** #161 asks to plan the service and
...
> - **§2.3 — the third kind is `EXPENSE_KIND_OPERATIONAL`,** not `BILL`. (owner, 2026-07-21) All three
...
| | `revenue_service` | `expense_service` (#161) |
| --- | --- | --- |
...
| Reading | Verdict | |
| --- | --- | --- |
| **An outgoing bill/expense** — rent, electricity, subscriptions | ✅ **CHOSEN** | A cost kind here, alongside ads and payroll. `expense_service` owns it end to end. |
...
enum ExpenseKind {
  ...
  EXPENSE_KIND_ADS = 1;          // marketplace ads budget
  EXPENSE_KIND_PAYROLL = 2;      // wages paid to people
  EXPENSE_KIND_OPERATIONAL = 3;  // §2.1's "payment" — rent, electricity, internet, subscriptions
  EXPENSE_KIND_OTHER = 4;
}
```

> ✅ **`EXPENSE_KIND_OPERATIONAL`** (owner, 2026-07-21), not `PAYMENT` and not `BILL`.
>
...
flowchart LR
  ...
  P -->|CostList totals| C["expense_service<br/>Σ amount by kind"]
...
No RPC here is multi-step or cross-service, so **no `docs/services/expense_service/rpc.md` is needed** —
only the schema doc.
...
| --- | --- | --- |
| 1 | **Cost record — proto + schema + migration.** `ExpenseKind`, `ExpenseRecord`, the `expense_records` table, `docs/database-schema.md` section. | — |
...
**4a is the piece the original plan missed.** It touches `revenue_service`, not `expense_service`, and it
