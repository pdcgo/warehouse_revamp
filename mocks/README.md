...
| [selling/product-list.html](selling/product-list.html) | Products — **selling team** (the catalogue, with stock) | The team's own catalogue, redesigned around the question a seller actually has: *what can I sell today?* **Active / Archived tabs** — `deleted` already exists on the row but is unreachable, so archived products can now be seen and **Restored** (which can legitimately fail: archiving frees the SKU, so a re-used one blocks it, and the row says so). Each row carries **ready** and **ongoing** stock as **count *and* value at cost** — never a selling price, a product has none — with the uncosted gap named rather than hidden (#74). The **warehouse filter restates every figure** as that warehouse's instead of only hiding rows, because stock is held per warehouse. **Cross-sell** (another team ordering *our* product for its own shop) is a column: open or not, the **markup** they pay, and what they moved in 30d. One **Product** column drawn by the shared `ProductListItem` — no image column, no SKU column. Paginates. |
