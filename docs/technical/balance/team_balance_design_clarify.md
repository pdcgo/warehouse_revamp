...
6. **🆕 Where is the DEFAULT row edited?** ([Critique 23](#critique))
   ⚠ **This is the surviving half of the old Q6, not a re-argument of it.** The placement is settled.
   What the answer does not supply is a home for `counterparty_id = 0` — the row that is terms for
   every team without their own. A pair detail lives at `/liability/:counterpartyId`, and **team 0 is
   not a team**, so there is no page on which that section can ever appear.
   **→ I recommend the CREDITOR's own settings** — the default belongs to the team that grants credit,
   not to any pair, so one field group on the team page reading *"terms for any team without their
   own"*. The pair detail's section then shows the inherited value with a *"using the default"* marker
   until somebody overrides it, which is what an exception-to-a-rule should look like.
> ⛔ **The rename question is DELETED — `liability` stays.**
...
> ⛔ **The naming question is DELETED — both logs are named.**
> [two-logs-two-names](./team_balance_design_decision.md#two-logs-two-names): `liability_logs` and
