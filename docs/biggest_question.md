...
[half-finished-orders-are-found-from-the-order](business/order/context_clarify.md#half-finished-orders-are-found-from-the-order).
...
Q2, which is a pointer to order's half-finished-orders-are-found-from-the-order (was Q14) rather than an open question. The per-file table sums to the header.
...
| **7** | **🆕 Who owns the warehouse's ORDER FEE — and is it a flat amount or a percent with a cap?** The new [warehouse context](business/warehouse/context.md) gives `warehouse_service` a `fee_percent`, a `max_fee` and a `WarehouseFeeCalculate` RPC. ⛔ **That rate already exists, in another service, in another shape, and it is charging money**: `liability_terms.handling_fee` is **flat rupiah per order**, held per creditor→debtor **pair** with `counterparty_id = 0` as the default row, and [`order_fees.go`](../backend/services/liability_service/liability_v1/order_fees.go) computes the charge from it when the `OrderPlaced` push arrives. So four things are undecided at once: **where the rate lives** · **what the percent is a percent OF** (an order has `subtotal`, `shipping_cost`, `total`, `cogs` and `marketplace_total`, all different, and `marketplace_total` is `0` on a phone order) · **whether the pair grain survives** a doc that describes one config per warehouse · and **whether the amount is FROZEN on the order** — `orders.warehouse_fee` is promised by the order context and **exists nowhere in the build**, so today a rate edited between commit and push delivery charges a number the order never saw. ⚠ **It also overtakes an in-flight change**: [technical/balance Q8](technical/balance/team_balance_design_clarify.md#question) asks whether to rename `handling_fee` → `order_fee`, which should not be done to a column that is about to move services. ⚠ **And the same doc's other half is dead weight in the build**: `warehouse_infos.receiving_hours` — the open/close grid — has a table, a proto, an editor and a test, and **no caller**, so an order can be placed into a warehouse that is shut. 🆕 **The payload arrived the same day and settles the shape while sharpening the types**: ✅ it takes a **basis** (`order_total`), not an `order_id`, and echoes the rate it used — both as recommended. ⛔ But every money field in it is a **`double`**, in a repo whose protos contain **no float anywhere** (`int64` whole rupiah, stated as a rule in `expense.proto`) · the request carries **no selling team**, so the per-pair rate cannot be expressed and the message has **no `use_scope` field** for the access interceptor · and it prices one warehouse per call while echoing `warehouse_id` like a batch. | ⛔ the whole of a new `warehouse_service` (nothing of it can be designed until the rate's owner is settled) · every order fee charged from today's flat terms · the `handling_fee` → `order_fee` rename, which would land on a doomed column | [warehouse: who-owns-the-rate-after-this](business/warehouse/context_clarify.md#who-owns-the-rate-after-this) · [what-is-the-fee-a-percent-of](business/warehouse/context_clarify.md#what-is-the-fee-a-percent-of) · [does-the-order-freeze-the-fee](business/warehouse/context_clarify.md#does-the-order-freeze-the-fee) · [Contradiction](business/warehouse/context_clarify.md#the-fee-rate-would-exist-in-two-services) | **The rate MOVES to `warehouse_service`, the order FREEZES the amount, and liability posts what it is handed** — the same shape recommended for the cross markup in **#5**, and for the same reason: a rate stored in two services is a quote and an invoice that disagree. Concretely: `fee_bp` + `min_fee` + `max_fee` in basis points and `int64` rupiah (never a float — `product_markup_bp` already sets that precedent), kept at the **pair** grain so today's per-seller rates survive · a migration writing each existing flat fee as `fee_bp = 0, min_fee = max_fee = <the flat fee>`, which reproduces today's charge exactly · `WarehouseFeeCalculate` **pure and batch**, taking a basis rather than an `order_id` (✅ the payload does this) so the order form can price three candidate warehouses before an order exists · **`int64` rupiah on every money field and `fee_bp` for the rate**, never `double`, with the rounding rule stated once in the RPC so the quoted, frozen and posted numbers are one number · **`team_id` in the request**, serving as both the pair key and the `use_scope` field the ACL needs · a **`configured` bool**, since `calculated_warehouse_fee = 0` otherwise means both *charges nothing* and *no rate set* · and `max_fee` **NULL = uncapped with `0` refused**, because `liability_terms.credit_limit` already carries that exact trap. ⚠ **Add `min_fee`**, which the doc omits: picking a Rp 10.000 order is the same work as a Rp 1.000.000 one, and 2.5% of it is Rp 250. |
