...
| **6** | **Where does a platform WITHDRAWAL live?** Wallet to bank, naming no order — so it is none of settlement's seven types, and [every-entry-names-an-order](business/settlement/context_decision.md#every-entry-names-an-order) made `order_id NOT NULL`, which forbids the obvious workaround. Asked in two docs, which is what this file exists to merge. | every shop's cash reconciliation — the wallet is fed by orders and drained by something with no home | [architecture Q7](technical/architecture/context_clarify.md#question) · [settlement Q3](business/settlement/context_clarify.md#question) | **Answer it once, in the architecture clarify, and have settlement follow.** → `order_service`, because the wallet is fed by that shop's orders and the withdrawal is reconciled against them. If a bank or cash account is ever modelled, it moves. |
| **7** | **⚠ RATIFY-ONLY — what moment consumes a FIFO layer, and which service stores the frozen cost?** ⬇ **was #4, demoted because the code has already taken this file's own recommendation** and nobody has asked the owner to ratify it. Shipped: layers are consumed at pick (`attributeDeltaFIFO`, [stock_pick.go:128](backend/services/inventory_service/inventory_v1/stock_pick.go#L128)), the cost is frozen at **commitment** ([order_place.go:204](backend/services/selling_service/selling_v1/order_place.go#L204)), and it is stored on `selling_service`'s order line rather than `ledger_service`'s. That is *at commitment, frozen on the line* — exactly the recommendation. ⛔ **Code taking a position is not the owner deciding one** (HARD RULE 8), so it stays open — but as a ratification, not a design. | the inventory/selling boundary and COGS correctness — both currently held by convention rather than by decision | [product Q5](business/product/context_clarify.md#question) · [architecture Q3](technical/architecture/context_clarify.md#question) | **Ratify what shipped, or say which part is wrong.** One line either way, and it converts a convention into a decision the next agent cannot quietly re-litigate. |
