...
>
> 🆕 **A RENAME is on the table and it folds into #3.** *Is it `balance_logs`, not `liability_entries`?*
> Agreed that *liability* must go — it names one leg of a **mirrored pair**, so half the rows are
> misnamed. Disagreed about *log*: a log's rows stand alone, and here two rows are one posting.
> **→ `balance_entries`, and the whole context** ([technical Q8](technical/balance/team_balance_design_clarify.md#question)).
> It is not a new item on this list — it rides #3, which is unrun and touches the same table.
>
...
| **3** | **⛔ DECIDED, NO LONGER BLOCKED, and it has GROWN — the ledger vocabulary migration, now carrying two more approved changes.** Not an open question: [the-ledger-speaks-the-business-words](business/balance/context_decision.md#the-ledger-speaks-the-business-words) settles the source types — `order_fee` · **`incidental_fee`** · `broken_good` · `lost_good` · `found` — and two later decisions land on the same table: **`actor_id`** ([every-entry-names-who-posted-it](technical/balance/team_balance_design_decision.md#every-entry-names-who-posted-it)) and, pending one answer, the **rename off *liability*** ([technical Q8](technical/balance/team_balance_design_clarify.md#question)). Three changes, one pass over the service. ✅ **The tooling block is GONE** (2026-08-29): `buf.gen.yaml` now uses `local:` plugins pinned by the root go.mod's `tool` directives and by frontend/package.json, so `cd proto && buf generate` needs **no Buf account**. It stays in this list only because it is the highest-value BUILD item in the repo and nothing has run it yet. | a live bug: the daily statement reads `COD_FEE`, which nothing posts, so its column is permanently zero while `RESTOCK_OUTLAY` appears in none · **and every day of waiting adds ledger rows that can never say who posted them** | [decision](business/balance/context_decision.md#the-ledger-speaks-the-business-words) · [technical balance C15](technical/balance/team_balance_design_clarify.md#critique) | **Run it once, with all three changes in it.** ⚠ Waiting costs history twice over and neither loss is recoverable: existing rows cannot say whether a `stock_damage` was **broken** or **lost**, and none of them can say **who posted it**. ⚠ Answer [technical Q8](technical/balance/team_balance_design_clarify.md#question) first, or the rename becomes a second pass — and after go-live it is a data migration plus a breaking proto change plus a client rollout. |
