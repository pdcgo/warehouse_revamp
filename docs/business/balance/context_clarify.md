...
12. **🆕 Must an incidental cost line carry a NOTE?** ([Critique 20](#critique))
    ⛔ **This is the last thing blocking a migration you already approved.**
    [the-ledger-speaks-the-business-words](./context_decision.md#the-ledger-speaks-the-business-words)
    collapses `COD_SHIPPING` + `OTHER` into one `INCIDENTAL` cost kind. Today the note is **optional
    for COD_SHIPPING** — the kind said what the money was — and **required for OTHER**. With one kind
    there is nothing left for that rule to key on.
    **→ I recommend REQUIRED, always.** Once the kind no longer says what the money was, the note is
    the only thing that does — and §Why `cod_fee` Exists describes this money as the courier's
    *accidental* ask, which is exactly the case a charged team cannot argue with unless somebody wrote
    down what it was for. The cost is one more required field at acceptance.
