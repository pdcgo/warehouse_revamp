...
[systems_product_context.md](./systems.md) is still
**one line**: `# Stock System Requirements.` — in a file named *product*, linked from
[product_context.md](./context.md) §System Requirements.
...
Siblings: [product_context](./context_clarify.md) · [stock_context](../stock/context_clarify.md).
...
| | Problem | → Recommend |
| --- | --- | --- |
| **1** | **The filename and the heading name two different subjects, and `product_context.md` links here as its own system requirements.** A reader following the link from the pricing doc lands on a stock heading. One of the two is wrong and only you know which. | If this is meant to be the **product** one, the heading is the typo. If the **stock** system requirements were started here by accident, they want their own `systems_stock_context.md` — and note [stock_context.md](../stock/context.md) is itself still one section long, so a system doc for it would be derived from very little ([stock_context_clarity](../stock/context_clarify.md)). |
| **2** | **`systems/` has no stated contract — and it is now the MIDDLE of a three-layer ladder.** A third layer, [`architectures/`](../../technical/architecture/context.md), arrived while this one was still a single empty heading, so any new rule now has three plausible homes and no stated rule for choosing. | **Moved, not dropped.** The boundary question is asked once, over the whole ladder, in [architectures/architecture_context_clarity](../../technical/architecture/context_clarify.md#question) — it changed shape when the third layer appeared and only the whole ladder can answer it. |
...
> [architectures/architecture_context_clarity](../../technical/architecture/context_clarify.md#question).
