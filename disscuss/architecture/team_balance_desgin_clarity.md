...
Critique, questions and a proposed shape for [team_balance_desgin.md](team_balance_desgin.md) — still
one heading — read against [balance_context.md](../../docs/requirements/balance_context.md),
[business_level.md](../../docs/requirements/business_level.md) and
[product_context.md](../../docs/requirements/product_context.md). Those docs are yours; this one is mine.
...
> [`disscuss/requirements/balance_context_clarity.md`](../requirements/balance_context_clarity.md), and
> the causes-list contradiction is recorded in
> [`business_level_clarity.md`](../requirements/business_level_clarity.md#the-balance-causes-list-is-written-twice-and-the-two-copies-differ).
...
## ✅ The markup is a FLOAT percent (owner) — and one plan doc now says otherwise

> `plans/settlement_service/brainstorming.md` §5.2.2: *"The markup is BASIS POINTS, not a float"*

That line is now stale and is the only site this decision leaves behind. **→ Recommend** it be updated
there when that plan is next touched, rather than left as a second answer to a settled question.
...
2. **Which service owns this ledger?** [`plans/settlement_service/brainstorming.md`](../../plans/settlement_service/brainstorming.md)
   already designs a team-to-team money ledger with screens and credit limits, and [`cost_design.md`](cost_design.md)
   is a second ledger keyed by `team_id`. **→ I recommend ONE ledger — this design, in `settlement_service`** —
   three services holding team money is three that can disagree. `cost_design.md` stays separate only if a cost
   never moves a balance ([its clarity Q2](cost_design_clarity.md) asks the same thing and is still open).
