package cost_service_models

import "time"

// CostRecord is a row of `cost_records` (#161) — money the business spent that NO ORDER caused.
//
...
type CostRecord struct {
	...
	// CostKind's enum number. Mapped in the handler — proto enums are open, so the database does not
