...
func list(t *testing.T, svc *cost_v1.Service, req *costv1.CostListRequest) *costv1.CostListResponse {
	t.Helper()
	...
	res, err := svc.CostList(context.Background(), connect.NewRequest(req))
	if err != nil {
		t.Fatalf("CostList: %v", err)
	}
...
func TestCostList_FiltersByPeriodAndTotalsTheWholeOfIt(t *testing.T) {
	...
	record(t, svc, costv1.CostKind_COST_KIND_ADS, 2_000_000, "2026-07-03")
	record(t, svc, costv1.CostKind_COST_KIND_ADS, 1_000_000, "2026-07-10")
	record(t, svc, costv1.CostKind_COST_KIND_PAYROLL, 12_000_000, "2026-07-05")
	record(t, svc, costv1.CostKind_COST_KIND_PAYROLL, 11_000_000, "2026-06-05")

	got := list(t, svc, &costv1.CostListRequest{
		TeamId: teamA,
		From:   "2026-07-01",
		To:     "2026-07-31",
	...
	if on := got.GetCosts()[0].GetOccurredAt(); on != "2026-07-10" {
	...
	if byKind[int32(costv1.CostKind_COST_KIND_ADS)] != 3_000_000 {
		t.Fatalf("ads total = %d, want 3000000", byKind[int32(costv1.CostKind_COST_KIND_ADS)])
	}
	if byKind[int32(costv1.CostKind_COST_KIND_PAYROLL)] != 12_000_000 {
		t.Fatalf("payroll total = %d, want 12000000", byKind[int32(costv1.CostKind_COST_KIND_PAYROLL)])
	}
	...
	if _, present := byKind[int32(costv1.CostKind_COST_KIND_OPERATIONAL)]; present {
...
func TestCostList_ThePeriodIncludesBothEnds(t *testing.T) {
	...
	record(t, svc, costv1.CostKind_COST_KIND_ADS, 100, "2026-07-01")
	record(t, svc, costv1.CostKind_COST_KIND_ADS, 200, "2026-07-31")

	got := list(t, svc, &costv1.CostListRequest{TeamId: teamA, From: "2026-07-01", To: "2026-07-31"})
...
func TestCostList_NoPeriodMeansEverything(t *testing.T) {
	...
	record(t, svc, costv1.CostKind_COST_KIND_ADS, 100, "2026-07-01")
	record(t, svc, costv1.CostKind_COST_KIND_PAYROLL, 200, "2020-01-01")

	got := list(t, svc, &costv1.CostListRequest{TeamId: teamA})
...
func TestCostList_ScopedToItsTeam(t *testing.T) {
	...
	record(t, svc, costv1.CostKind_COST_KIND_PAYROLL, 9_000_000, "2026-07-05")

	got := list(t, svc, &costv1.CostListRequest{TeamId: 3})

	if n := len(got.GetCosts()); n != 0 {
		t.Fatalf("another team's costs leaked: %v", got.GetCosts())
	}
...
func TestCostList_FiltersByKind(t *testing.T) {
	...
	got := list(t, svc, &costv1.CostListRequest{
		TeamId: teamA, Kind: costv1.CostKind_COST_KIND_ADS,
	})
