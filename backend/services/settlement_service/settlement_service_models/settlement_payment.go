...
// SettlementPayment is a row of `settlement_payments` (#188) — one team's claim that it paid another,
// and the creditor's agreement that the money arrived.
//
// ⚠ IT IS NOT THE LEDGER. This table records the claim; `settlement_entries` records what moved. A
...
type SettlementPayment struct {
	...
	// reason `settlement_entries.source_type` is.
	Status string
...
func (SettlementPayment) TableName() string {
