...
// SettlementDaily sums what the ledger moved PER DAY, from the scoped team's point of view.
//
...
// picks — see the note on SettlementDailyFilter for why summing all four would double-count.
//
// ⚠ ONE GROUPED QUERY, not a query per day.
func (s *Service) SettlementDaily(
	ctx context.Context,
	req *connect.Request[settlementv1.SettlementDailyRequest],
) (*connect.Response[settlementv1.SettlementDailyResponse], error) {
	...
	return connect.NewResponse(&settlementv1.SettlementDailyResponse{
		Days:   days,
		Totals: totals,
	}), nil
}

// settlementDayRow is the grouped query's shape — one row per (day, source_type).
//
...
func foldDays(rows []settlementDayRow) ([]*settlementv1.SettlementDayItem, *settlementv1.SettlementDailyTotals) {
	days := make([]*settlementv1.SettlementDayItem, 0, len(rows))
	totals := &settlementv1.SettlementDailyTotals{BySource: map[int32]int64{}}

	var current *settlementv1.SettlementDayItem

	for i := range rows {
		date := rows[i].Day.Format(dateLayout)

		if current == nil || current.GetDate() != date {
			current = &settlementv1.SettlementDayItem{
				Date:     date,
