...
// Concurrency audit for SettlementPaymentConfirm (the audit-sql skill).
//
...
//
//	go test -tags raceaudit -run TestRace_SettlementPaymentConfirm -v ./backend/services/settlement_service/settlement_v1/
...
func TestRace_SettlementPaymentConfirm(t *testing.T) {
	...
	_, err := svc.PostEntry(ctx, nil, settlement_v1.Posting{
	...
	recorded, err := svc.SettlementPaymentRecord(ctx,
		connect.NewRequest(&settlementv1.SettlementPaymentRecordRequest{
			TeamId:         selling,
			CreditorTeamId: warehouse,
			Amount:         15000,
		}))
	...
	res := h.Race(t, 8, func(int) error {
		_, confirmErr := svc.SettlementPaymentConfirm(ctx,
			connect.NewRequest(&settlementv1.SettlementPaymentConfirmRequest{
				TeamId:    warehouse,
				PaymentId: paymentID,
			}))
	...
	err = db.Raw(`SELECT balance FROM settlement_balances WHERE team_id = ? AND counterparty_id = ?`,
	...
	err = db.Raw(`SELECT status FROM settlement_payments WHERE id = ?`, paymentID).Scan(&status).Error
	if err != nil {
...
func TestRace_SettlementPaymentConfirmAgainstReverse(t *testing.T) {
	h := san_race.New(t, paymentTables...)
	db := h.DB()
	svc := settlement_v1.NewService(db)
	ctx := context.Background()
	...
	h.Race(t, 8, func(i int) error {
		...
		_, reverseErr := svc.SettlementPaymentReverse(ctx,
