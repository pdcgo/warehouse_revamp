...
) *settlementv1.SettlementPaymentListResponse {
	t.Helper()

	res, err := svc.SettlementPaymentList(context.Background(),
		connect.NewRequest(&settlementv1.SettlementPaymentListRequest{
			TeamId: teamID,
			Filter: filter,
			Page:   &commonv1.CommonPagination{Page: 1, Limit: 50},
		}))
	if err != nil {
		t.Fatalf("SettlementPaymentList(%d): %v", teamID, err)
	}
...
func TestPaymentList_AwaitingMyConfirmationIsOnlyWhatIOwe(t *testing.T) {
	...
	msg := paymentList(t, svc, warehouse,
		&settlementv1.SettlementPaymentListFilter{AwaitingMyConfirmation: true})
...
func TestPaymentList_CounterpartyFilterNarrows(t *testing.T) {
	...
	msg := paymentList(t, svc, selling,
		&settlementv1.SettlementPaymentListFilter{CounterpartyId: warehouse})
...
func TestPositionList_BadgeCountsWaitingPayments(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("SettlementPositionList: %v", err)
	}
