...
func recordPayment(
	...
	amount int64,
) *settlementv1.SettlementPayment {
	t.Helper()

	res, err := svc.SettlementPaymentRecord(context.Background(),
	...
	if err != nil {
		t.Fatalf("SettlementPaymentRecord(%d -> %d): %v", payer, creditor, err)
	}
...
func confirmPayment(
	...
	res, err := svc.SettlementPaymentConfirm(context.Background(),
...
func TestPaymentConfirm_SettlesTheDebt(t *testing.T) {
	...
	if got.GetStatus() != settlementv1.SettlementPaymentStatus_SETTLEMENT_PAYMENT_STATUS_CONFIRMED {
...
func TestPaymentConfirm_TheEntryNamesThePayment(t *testing.T) {
	...
	if !found {
		t.Fatal("no PAYMENT entry in the history — the settlement is untraceable")
	}
}
