...
func deleteTerms(t *testing.T, svc *settlement_v1.Service, teamID, counterpartyID uint64) {
	t.Helper()

	_, err := svc.SettlementTermsDelete(context.Background(),
		connect.NewRequest(&settlementv1.SettlementTermsDeleteRequest{
			TeamId:         teamID,
			CounterpartyId: counterpartyID,
		}))
	if err != nil {
		t.Fatalf("SettlementTermsDelete(%d -> %d): %v", teamID, counterpartyID, err)
	}
}
