...
// SettlementPaymentConfirm is PHASE TWO (#188): the creditor saw the money arrive. THIS is what posts.
//
...
// learn it.
func (s *Service) SettlementPaymentConfirm(
	ctx context.Context,
	req *connect.Request[settlementv1.SettlementPaymentConfirmRequest],
) (*connect.Response[settlementv1.SettlementPaymentConfirmResponse], error) {
	var payment settlement_service_models.SettlementPayment
	...
	err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		updateErr := tx.Model(&settlement_service_models.SettlementPayment{}).
			Where("id = ?", found.ID).
	...
	return connect.NewResponse(&settlementv1.SettlementPaymentConfirmResponse{
...
func paymentPosting(p *settlement_service_models.SettlementPayment, reversal bool) Posting {
	return Posting{
...
) (*settlement_service_models.SettlementPayment, error) {
	var found settlement_service_models.SettlementPayment

	err := tx.
		Raw("SELECT * FROM settlement_payments WHERE id = ? AND creditor_team_id = ? FOR UPDATE",
