...
func putTerms(
	...
	limit *int64,
) *settlementv1.SettlementTerms {
	t.Helper()

	res, err := svc.SettlementTermsSet(context.Background(),
		connect.NewRequest(&settlementv1.SettlementTermsSetRequest{
			TeamId:          teamID,
	...
	if err != nil {
		t.Fatalf("SettlementTermsSet(%d -> %d): %v", teamID, counterpartyID, err)
	}
...
func TestTermsSet_RefusesTermsAgainstYourself(t *testing.T) {
	...
	_, err := svc.SettlementTermsSet(context.Background(),
		connect.NewRequest(&settlementv1.SettlementTermsSetRequest{
			TeamId:         warehouse,
