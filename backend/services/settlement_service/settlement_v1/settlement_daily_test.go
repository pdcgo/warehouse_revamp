...
const (
	handlingFeeSource = int32(settlementv1.SettlementSourceType_SETTLEMENT_SOURCE_TYPE_HANDLING_FEE)
	codFeeSource      = int32(settlementv1.SettlementSourceType_SETTLEMENT_SOURCE_TYPE_COD_FEE)
)
...
func daily(
	...
	teamID uint64,
	from, to string,
) *settlementv1.SettlementDailyResponse {
	t.Helper()

	res, err := svc.SettlementDaily(context.Background(),
		connect.NewRequest(&settlementv1.SettlementDailyRequest{
			TeamId: teamID,
			Filter: &settlementv1.SettlementDailyFilter{From: from, To: to},
		}))
	if err != nil {
		t.Fatalf("SettlementDaily(%d, %s..%s): %v", teamID, from, to, err)
	}
...
func handling(amount int64, sourceID uint64) settlement_v1.Posting {
	return settlement_v1.Posting{
		DebtorTeamID:   selling,
		CreditorTeamID: warehouse,
		Amount:         amount,
		SourceType:     settlement_v1.SourceTypeHandlingFee,
		SourceID:       sourceID,
	}
}
...
func postOn(
	...
	err = db.Exec(`UPDATE settlement_entries SET created_at = ? WHERE group_id = ?`, at, group).Error
	if err != nil {
...
// disagree with it.
func TestSettlementDaily_GroupsByDayAscendingWithTheSourceSplit(t *testing.T) {
	db := san_testdb.DB(t)
	...
	productFee := int32(settlementv1.SettlementSourceType_SETTLEMENT_SOURCE_TYPE_PRODUCT_FEE)
...
// happened.
func TestSettlementDaily_AReversedFeeNetsTheDayToZero(t *testing.T) {
	db := san_testdb.DB(t)
	svc := settlement_v1.NewService(db)
...
// else's statement.
func TestSettlementDaily_TheTwoSidesAreExactNegatives(t *testing.T) {
	db := san_testdb.DB(t)
	svc := settlement_v1.NewService(db)
...
func TestSettlementDaily_TheLastDayOfThePeriodCountsInFull(t *testing.T) {
	db := san_testdb.DB(t)
	svc := settlement_v1.NewService(db)
...
func TestSettlementDaily_RefusesAnUnboundedOrOversizedPeriod(t *testing.T) {
	db := san_testdb.DB(t)
	svc := settlement_v1.NewService(db)
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			_, err := svc.SettlementDaily(context.Background(),
				connect.NewRequest(&settlementv1.SettlementDailyRequest{
					TeamId: warehouse,
					Filter: &settlementv1.SettlementDailyFilter{From: tc.from, To: tc.to},
				}))
			if err == nil {
				t.Fatalf("SettlementDaily(%q..%q) succeeded — an unbounded series is the bug the cap exists to stop", tc.from, tc.to)
			}

			if code := connect.CodeOf(err); code != connect.CodeInvalidArgument {
				t.Fatalf("SettlementDaily(%q..%q) = %v, want InvalidArgument", tc.from, tc.to, code)
			}
		})
	}
}
...
func TestSettlementDaily_NeverLeaksAnotherTeamsDays(t *testing.T) {
	db := san_testdb.DB(t)
	svc := settlement_v1.NewService(db)
	...
	res, err := svc.SettlementDaily(context.Background(),
		connect.NewRequest(&settlementv1.SettlementDailyRequest{
			TeamId: stranger,
			Filter: &settlementv1.SettlementDailyFilter{From: "2026-07-01", To: "2026-07-31"},
		}))
	if err != nil {
		t.Fatalf("SettlementDaily for a stranger: %v", err)
	}
...
func TestSettlementDaily_AnEmptyPeriodIsZeroNotNil(t *testing.T) {
	db := san_testdb.DB(t)
	svc := settlement_v1.NewService(db)
