...
// SettlementPaymentList reads the payments this team is a party to (#188) — BOTH DIRECTIONS, the ones
...
func (s *Service) SettlementPaymentList(
	ctx context.Context,
	req *connect.Request[settlementv1.SettlementPaymentListRequest],
) (*connect.Response[settlementv1.SettlementPaymentListResponse], error) {
	...
	var rows []settlement_service_models.SettlementPayment
	...
	out := make([]*settlementv1.SettlementPayment, 0, len(rows))
	for i := range rows {
	...
	return connect.NewResponse(&settlementv1.SettlementPaymentListResponse{
		Items: items,
		Ids:   ids,
...
func (s *Service) paymentScope(
	ctx context.Context,
	teamID uint64,
	filter *settlementv1.SettlementPaymentListFilter,
) *gorm.DB {
	query := s.db.
		WithContext(ctx).
