...
-- could never confirm.
--
-- ⚠ THIS TABLE IS NOT THE LEDGER. It is the claim-and-agreement record; `liability_entries` is what
...
CREATE TABLE liability_payments (
    ...
    -- `liability_entries.source_type` is: a DB CHECK IN-list is one more place to drift when the
    -- enum grows (cf. #80).
    ...
    CONSTRAINT liability_payments_no_self CHECK (payer_team_id <> creditor_team_id),
    ...
    CONSTRAINT liability_payments_positive CHECK (amount > 0)
);
...
CREATE INDEX liability_payments_awaiting_idx
    ON liability_payments (creditor_team_id, status, id DESC);

-- The payer's own history, for the same screen read from the other side.
CREATE INDEX liability_payments_payer_idx
    ON liability_payments (payer_team_id, id DESC);
-- +goose StatementEnd

-- +goose Down
-- +goose StatementBegin
DROP TABLE liability_payments;
-- +goose StatementEnd
