...
--
-- Two tables, and the relationship between them is the whole design: `liability_entries` is the
-- TRUTH — immutable, append-only — and `liability_balances` is a PROJECTION of it. Every balance
...
CREATE TABLE liability_entries (
    ...
    CONSTRAINT liability_entries_sides_differ CHECK (team_id <> counterparty_id)
);
...
CREATE UNIQUE INDEX liability_entries_source_idx
    ON liability_entries (team_id, counterparty_id, source_type, source_id, reversal);

-- The history screen: one counterparty's entries, newest first.
CREATE INDEX liability_entries_pair_idx ON liability_entries (team_id, counterparty_id, id DESC);
...
CREATE SEQUENCE liability_group_seq;
...
-- it was first needed.
CREATE UNIQUE INDEX liability_balances_pair_idx
    ON liability_balances (team_id, counterparty_id);
-- +goose StatementEnd

-- +goose Down
-- +goose StatementBegin
DROP TABLE liability_balances;
DROP SEQUENCE liability_group_seq;
DROP TABLE liability_entries;
-- +goose StatementEnd
