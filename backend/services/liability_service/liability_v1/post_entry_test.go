...
func codFee(amount int64, sourceID uint64) liability_v1.Posting {
...
func balanceOf(t *testing.T, db *gorm.DB, teamID, counterpartyID uint64) int64 {
	...
	var row liability_service_models.LiabilityBalance
...
func TestPostEntry_OneSourceCanOweSeveralCounterparties(t *testing.T) {
	...
	_, err = post(t, svc, db, liability_v1.Posting{
		...
		SourceType: liability_v1.SourceTypeProductFee, SourceID: 412,
	})
	if err != nil {
...
func TestPostEntry_TheAgeingClockStartsAndStops(t *testing.T) {
	...
	if settled.OldestUnsettledAt != nil {
		t.Fatalf("the ageing clock survived a full liability (%v)", settled.OldestUnsettledAt)
	}
}
...
) liability_service_models.LiabilityBalance {
	t.Helper()
