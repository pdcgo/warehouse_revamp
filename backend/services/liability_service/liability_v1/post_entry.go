...
var ErrAlreadyPosted = errors.New("liability: this movement is already posted")
...
func (s *Service) PostEntry(ctx context.Context, tx *gorm.DB, p Posting) (uint64, error) {
	if p.DebtorTeamID == 0 || p.CreditorTeamID == 0 {
		return 0, errors.New("liability: a posting needs both sides")
	}
	...
	if p.Amount <= 0 {
		return 0, errors.New("liability: a posting must be a positive amount")
	}

	sourceType := sourceTypeText(p.SourceType)
	if sourceType == "" {
		return 0, errors.New("liability: a posting must say what caused it")
	}
...
func (s *Service) postBothLegs(tx *gorm.DB, p Posting, sourceType string) (uint64, error) {
	...
	err := tx.Raw("SELECT nextval('liability_group_seq')").Scan(&groupID).Error
	if err != nil {
		return 0, err
	}
...
ON CONFLICT (team_id, counterparty_id) DO UPDATE SET
    balance = liability_balances.balance + EXCLUDED.balance,
    oldest_unsettled_at = CASE
        -- Square again: stop the clock.
        WHEN liability_balances.balance + EXCLUDED.balance = 0 THEN NULL
        -- Debt begins here: start it.
        WHEN liability_balances.balance = 0 THEN EXCLUDED.oldest_unsettled_at
        -- More debt on an existing run: the oldest unsettled is still the oldest.
        ELSE liability_balances.oldest_unsettled_at
    END,
