...
func termsList(
	...
	teamID uint64,
) *liabilityv1.LiabilityTermsListResponse {
	t.Helper()

	res, err := svc.LiabilityTermsList(context.Background(),
		connect.NewRequest(&liabilityv1.LiabilityTermsListRequest{
			TeamId: teamID,
			Page:   &commonv1.CommonPagination{Page: 1, Limit: 50},
		}))
	if err != nil {
		t.Fatalf("LiabilityTermsList(%d): %v", teamID, err)
	}
...
func termsRows(msg *liabilityv1.LiabilityTermsListResponse) map[uint64]*liabilityv1.LiabilityTerms {
	for _, item := range msg.GetItems() {
		if terms, ok := item.GetD().(*liabilityv1.LiabilityTermsListResponseItem_Terms); ok {
